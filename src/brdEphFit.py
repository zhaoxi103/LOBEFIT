...
from brdParDeriv import formMatrixManyEpochs, formMatrixManyEpochs_parallel
...
def ephfit(time_arr, orbit_arr, optClass):
    ...
    for i in range(nMaxIter):
        if np.sum(np.square(deltaV)) >= 1e-7:
            ...
            # A_mat, B_mat = formMatrixManyEpochs_parallel(chosen_orbit_arr, x_brd0, tk_arr, toe, npara, para_case)
            #
...
def readConfig(cfgPath, opt):
    ...
    _fittingTime = GetIniKeyString("config", "Fitting_Time", cfgPath)
    ...
    opt.fitNum = int(int(_fittingTime) / opt.inter_proc) + 1
    ...
    if sp3Type == 'P':
        ...
        for k in range(len(x_sp3_pool)):
            if k == 0 or k == 1:
                ...
                x_sp3_pool[k, 3:6] = (x_sp3_pool[k - 2, 0:3] - 8 * x_sp3_pool[k - 1, 0:3] + 8 * x_sp3_pool[k + 1,
                                                                                                0:3] - x_sp3_pool[k + 2,
                                                                                                       0:3]) / (
                                             12 * sampleTime)
